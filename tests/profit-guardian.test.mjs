...
describe('_trailingTakeProfit — high-lev runners (arm at +14% margin, exit on 2% pullback)', () => {
  ...
  test('thresholds: arm at 11% NET margin, trail by 2%', () => {
    const { app } = loadApp();
    assert.equal(app.TRAIL_ARM_NET_MARGIN_PCT, 11);
    assert.equal(app.TRAIL_FROM_PEAK_MARGIN_PCT, 2);
  });

  test('does not arm when peak margin is below 11% NET', async () => {
    const { app, sandbox } = loadApp();
    liveSetupHighLev(app, sandbox);
    // Long entry 100, mark 100.05 → +0.05% price × 200× = +10% gross → -6% net
    // (after 16% fee burden). Below the +11% arm threshold.
    ...
    assert.equal(app._trailState.SOL?.armed, false, 'must not arm below +11% NET margin');
    assert.equal(closeBodies.length, 0, 'no close call');
  });

  test('arms when long position touches +11% NET margin (= +0.135% price at 200×)', async () => {
    const { app, sandbox } = loadApp();
    liveSetupHighLev(app, sandbox);
    ...
    sandbox.fetch = async () => ({ ok: true, status: 200, text: async () => '{"success":true,"code":0}' });
    await app._trailingTakeProfit();
    assert.equal(app._trailState.SOL.armed, true, 'should arm — peak NET margin clears +11%');
    assert.ok(app._trailState.SOL.peakNetMarginPct >= 11, `peak ≥ 11%, got ${app._trailState.SOL.peakNetMarginPct}`);
  });
